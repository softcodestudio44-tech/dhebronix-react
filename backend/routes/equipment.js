...
router.post('/', verifyAdmin, async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [name, category, price, oldPrice || null, condition || 'new', brand || '', specs || '', description, images || [], available !== false]
    );
...
router.put('/:id', verifyAdmin, async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [name, category, price, oldPrice || null, condition || 'new', brand || '', specs || '', description, images || [], available !== false, req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Equipment not found' });
    res.json(result.rows[0]);
  } catch (error) {
