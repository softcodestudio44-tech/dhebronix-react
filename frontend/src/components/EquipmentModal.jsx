...
import { X, ShoppingCart, Tag, Box, Info } from 'lucide-react'
    ...
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-3xl">
      <div className="grid md:grid-cols-2 gap-0">
        ...
        <div className="p-6 md:p-8">
          <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">{product.brand || 'DHEBRONIX'}</div>
          <h2 className="font-orbitron text-2xl font-bold text-white mb-4">{product.name}</h2>
          
          <div className="flex items-center gap-3 mb-6">
            <span className="text-3xl font-bold text-primary-light">₦{product.price?.toLocaleString()}</span>
            {product.old_price && (
              <span className="text-gray-500 text-lg line-through">₦{product.old_price?.toLocaleString()}</span>
            )}
          </div>
